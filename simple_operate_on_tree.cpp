...
void addNodeIntoTree(TNODE* root, char data)
{
    ...
    { // TH2. root đã có firstChild, cần tìm vị trí con nhỏ nhất hiện tại
...
TNODE* findKey(TNODE* root, char key) {
    ...
    TNODE* childResult = findKey(root->firstChild, key);
...
{
    TNODE* parentnode = findKey(root, parentLabel);
    if (parentnode != NULL)
    {
        TNODE* childnode = makeNewNode(newChildLabel);
        if  (parentnode->firstChild == NULL) parentnode->firstChild = childnode;
        else
        {
            TNODE* preNode = parentnode->firstChild;
            while (preNode->nextSibling != NULL) {
                preNode = preNode->nextSibling;
            }
            preNode->nextSibling = childnode;
        }
    }
}
...
{
    TNODE* branchroot = findKey(root, branchRootLabel);
    if (branchroot == NULL) return;
    TNODE* parent = findParent(root, branchRootLabel);
    if (parent == NULL) return;

    if (parent->firstChild == branchroot) parent->firstChild = branchroot->nextSibling;
    else {
        TNODE* tmp = parent->firstChild;
        while (tmp->nextSibling != branchroot) {
            tmp = tmp->nextSibling;
        }
        tmp->nextSibling = branchroot->nextSibling;
    }
    branchroot->nextSibling = NULL;
    deleteTree(&branchroot);
}
